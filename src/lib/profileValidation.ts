...
export function validateProfileCompletion(user: UserProfile): ProfileValidationResult {
  if (!user || !user.role) {
    return { isComplete: false, missingFields: ['role'], requiredFields: ['role'], optionalFields: [] };
  }
  ...
  if (missingBasicFields.length > 0) {
    return { isComplete: false, missingFields: missingBasicFields, requiredFields: basicRequiredFields, optionalFields: [] };
  }

  const roleConfig = PROFILE_FIELD_CONFIG[user.role as 'artist' | 'customer' | 'admin' | 'user'];
  
  if (!roleConfig) {
    return { isComplete: false, missingFields: ['valid_role'], requiredFields: ['valid_role'], optionalFields: [] };
  }

  if (user.role === 'user') {
    return { isComplete: false, missingFields: ['role_selection'], requiredFields: ['role_selection'], optionalFields: [] };
  }

  if (user.role === 'admin') {
    return { isComplete: true, missingFields: [], requiredFields: [], optionalFields: [] };
  }
...
export const FIELD_DISPLAY_NAMES: Record<string, string> = {
  name: 'Full Name', email: 'Email Address', phone: 'Phone Number', age: 'Age',
  height: 'Height', weight: 'Weight', color: 'Skin Color', ethnicity: 'Ethnicity',
  bio: 'Bio', specialties: 'Specialties', address: 'Street Address', addressLine2: 'Address Line 2',
  city: 'City', state: 'State', zipCode: 'ZIP Code', country: 'Country', gender: 'Gender',
  other: 'Additional Information', services: 'Services Offered', availableLocations: 'Available Locations',
  bookingInfo: 'Booking Information', portfolioLink: 'Portfolio Link', bookingPreferences: 'Booking Preferences',
  preferredArtists: 'Preferred Artists', companyName: 'Company/Studio Name', category: 'Category',
  experience: 'Experience', advanceBookingDays: 'Advance Booking Days', bookingType: 'Booking Type',
  paymentMethods: 'Payment Methods', allowPartialPayment: 'Allow Partial Payment'
};
