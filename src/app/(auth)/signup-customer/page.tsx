    ...
    if (signupError) {
      switch (signupError) {
        case 'google_signup_failed':
          setError("Google signup failed. Please ensure you've granted necessary permissions and try again.");
          break;
        case 'profile_creation_failed':
          setError("Could not create your customer profile after Google authentication. Please try again.");
          break;
        case 'email_already_exists_artist':
            setError("This email is already registered as an artist. Please log in or use a different email for a customer account.");
            break;
        case 'email_already_exists_customer':
            setError("This email is already registered as a customer. Please log in.");
            break;
        // Add other specific error cases from your backend
        default:
          setError("An unknown error occurred during the signup process. Please try again.");
      }
    } else {
      setError(null);
    }
...
// Define the background image paths (you can use the same ones or different ones)
...
export default function SignupCustomerPage() {
  ...
  const handleGoogleSignupCustomer = () => {
    setIsRedirecting(true);
    window.location.href = `${API_BASE_URL}/auth/google?role=customer`;
  };
    ...
    >
      ...
      <Card className="w-full max-w-md shadow-xl bg-[#161616]/80 border-[#2a2a2a] text-white relative z-10">
        <CardHeader className="space-y-1.5 border-b border-[#2a2a2a] pb-4">
          <CardTitle className="text-2xl font-bold text-center text-pink-600">Customer Signup</CardTitle>
          <CardDescription className="text-center text-gray-300 pt-1">
            Join Laaiqa to discover and purchase unique artwork.
          </CardDescription>
        </CardHeader>
        <CardContent className="py-6">
          ...
          <p className="text-sm text-gray-300 mb-6 text-center">
            Support artists and find pieces you love by signing up with Google.
          </p>
          <Button
            type="button"
            variant="outline"
            ...
            onClick={handleGoogleSignupCustomer}
          ...
          >
            ...
            {isRedirecting ? 'Processing...' : 'Sign up as Customer with Google'}
          </Button>
        </CardContent>
        <CardFooter className="flex flex-col space-y-3 pt-6 border-t border-[#2a2a2a]">
          ...
          <div className="text-center text-sm text-gray-400">
            ...
            <Link href="/signup-artist" className="font-medium text-pink-600 hover:text-pink-500 hover:underline">
              Sign up as Artist
            </Link>
          </div>
        </CardFooter>
