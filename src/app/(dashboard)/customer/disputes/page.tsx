...
export default function CustomerDisputesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const quotesRes = await fetch(`${API_BASE_URL}/api/quotes/customer`, { credentials: 'include' });
      if (quotesRes.ok) {
        const quotesData = await quotesRes.json();
        setQuotes(quotesData);
        // Extract disputes from quotes
  ...
  useEffect(() => {
    if (!authLoading && user) {
      if (user.role !== 'customer') {
        router.push(user.role === 'artist' ? '/artist' : '/');
        return;
      }
      fetchData();
  ...
  // Quotes eligible for dispute (not cancelled, has customer)
        ...
        {disputes.length === 0 ? (
          ...
          <div className="space-y-4">
            {disputes.map(dispute => {
              ...
              const raisedByArtist = dispute.initiatorRole === 'artist';
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {quote?.artistName && ` • ${quote.artistName}`}
                      </p>
                    </div>
        ...
        <DialogContent className="bg-[#1a1a1a] border-gray-800 text-white max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400">
              Raise a dispute for an issue with your booking
            </DialogDescription>
          </DialogHeader>
              ...
              >
                ...
                {eligibleQuotes.map(q => (
                  <option key={q.id} value={q.id}>
                    {q.productType} - {format(parseISO(q.serviceDate), 'dd MMM yyyy')}
                  </option>
                ))}
              </select>
            </div>
